...
/// Shown after ad creation when the backend requires payment.
///
/// The backend does not return the new ad's ID in the create-ad response,
/// so we fetch the latest unpaid/inactive ad on mount to resolve it.
class AdPaymentScreen extends StatefulWidget {
  final double price;

  /// Pass the adId if the backend returned one; leave empty ('') when it
  /// didn't — the screen will fetch it automatically.
  final String adId;
  ...
  Future<void> _resolveAdId() async {
    ...
    try {
      ...
      if (result.items.isNotEmpty) {
        ...
        setState(() {
          _resolveError =
              'Could not find the created ad. Please try again.';
          _isResolvingId = false;
        });
      }
    } catch (e) {
    ...
    return BlocProvider(
      ...
      child: BlocListener<PaymentBloc, PaymentState>(
        listener: (context, state) {
          // ── Credit card: open redirect URL in WebView ──────────────────
