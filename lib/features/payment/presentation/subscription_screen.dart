...
//   _dismissProcessingDialog();
//   // ✅ free plan → dialog ثم navigate لـ mainLayout
//   showDialog(
//     context: context,
//     barrierDismissible: false,
//     builder: (_) => PaymentSuccessDialog(
...
//   _dismissProcessingDialog();
//   // ✅ manual → dialog فقط بدون navigate (الـ VodafoneScreen هيعمل pop)
//   showDialog(
//     context: context,
//     barrierDismissible: false,
//     builder: (_) => PaymentSuccessDialog(
...
class _SubscriptionScreenState extends State<SubscriptionScreen>
          ...
          builder: (context, state) {
            return Stack(
              children: [
                ...
                // ── X close button (only if can pop) ──
                // if (canPop)
                  Positioned(
  ...
  Widget _buildBody(PaymentState state, S s) {
    if (state is PlansLoading || state is MySubscriptionLoading) {
      return SizedBox(
        height: 300.h,
        child: const Center(
          child: CircularProgressIndicator(color: Color(0xFF4CAF50)),
            ...
            children: [
              ...
              TextButton(
                ...
                child: Text(
                  s.subscription_retry,
                  style: TextStyle(
                      color: const Color(0xFF4CAF50), fontSize: 14.sp),
                ),
              ),
            ],
          ),
    ...
    final plans =
        state is PlansLoaded ? state.plans : <SubscriptionPlanModel>[];
                    ...
                    child: GestureDetector(
                      ...
                      child: Padding(
                        padding: EdgeInsets.only(
                          left: entry.key == 0 ? 0 : 5.w,
                          right:
                              entry.key == plans.length - 1 ? 0 : 5.w,
                        ),
                        child: _PlanCard(
                          plan: plan,
                          isSelected: isSelected,
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    ?Theme.of(context).colorScheme.onError
                        // ? const Color(0xFF4CAF50) // ✅ أخضر للـ current
                        : isSelected
                            ? _accent
                            : Colors.transparent,
                    borderRadius: BorderRadius.circular(8.r),
